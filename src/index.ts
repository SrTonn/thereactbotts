...
import {
  Bot, Context, session, SessionFlavor,
} from 'grammy'
...
// Create a bot
const bot = new Bot<MyContext>(process.env.BOT_TOKEN ?? '') // <-- place your token inside this string
...
bot.filter((ctx) => ctx.chat?.type === 'private').command('start', (ctx) => {
  ...
  ctx.reply(
    `Hi ${userName}! I will count the messages in this chat so you can get your /stats!`,
  )
})

bot.on(':new_chat_members:me', (ctx) => ctx.reply(
